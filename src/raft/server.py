...
class RaftServer:
    def __init__(self, server_no, num_servers, state_machine, persist=False):
        ...
        self._commit_index = -1 #последняя запись логов, которая может подойти
        self.last_applied = -1
        self.leader_id = None
        ...
        self.recover() #найти последние логи, чтобы восстановить состояния машины
    @property
    ...
    def handle_election_timeout(self): #from _event_loop() trhead
        if self.state == State.LEADER:
            pass
        self.become_candidate()

    @only(State.LEADER, silent=True) 
    def handle_heartbeat(self): #from _event_loop() trhead
        self.outbox.put("reset_election_timeout") #Мы должны сбросить всем таймеры до выборов

        followers = [
            server for server in range(self.num_servers) if server != self.server_no
        ]
        for server_no in followers: #для каждого подписчика делаем append_entries
            append_entries_msg = self._append_entries_msg(server_no)
            if append_entries_msg: #непонятно зачем это уловие, если всегда вернётся объект AppednEntries
    ...
    def handle_message(self, message: Message): #from _event_loop() trhead
        message_handlers = {
            AppendEntries: self._handle_append_entries,
            AppendEntriesSucceeded: self._handle_append_entries_succeeded,
            AppendEntriesFailed: self.handle_message,
    ...
    def _handle_request_vote(
        ...
        if term < self.term: #Если term, отправленный кандидатом, меньше, чем текущий term на сервере, сервер отклоняет голосование.
            return VoteDenied(
                f"Vote came from server on term {term} while own term was {self.term}"
            )

        if not (self.voted_for is None or self.voted_for == candidate_id): #Если сервер уже отдал свой голос на данном терме выборов, он не может голосовать еще раз и должен отклонить голосование
            return VoteDenied(f"already voted for other candidate: {self.voted_for}")

        if candidate_log_len < len(self.log): #Если кандидат имеет меньший журнал, чем у сервера, сервер отклонит голосование.
            return VoteDenied(
                f"candidate log was size {candidate_log_len} while own log length was {len(self.log)}"
            )

        if last_log_term < self.log.last_term: #Если последний термин в журнале кандидата меньше, чем последний термин в журнале сервера, сервер отклоняет голосование.
            return VoteDenied(
        ...
        return VoteGranted() #Если все условия выполнены, сервер сбрасывает таймер выборов и отдает голос за кандидата

    @only(State.LEADER)
    def _append_entries_msg(self, server_no): #цель AppendEntries: обновить журнал follower-ов на основе записей получченых от leader
    ...
    def _handle_append_entries(
        ...
        """
        Args:
            leader_id: so follower can redirect clients #для перенаправления клиентов фолловером
            log_index: index in the log where to append to #индекс в журнале, куда нужно добавить запись
            prev_log_term: according to the leader, the term of the last log entry #по словам лидера, термин последней записи в журнале
            entry: the entry to add #запись для добавления
            leader_commit: the leaders' commit_index #индекс последней записи, подтвержденной лидером
        ...
        # Минимум в окончательном шаге (5) AppendEntries необходим и должен быть вычислен с индексом последней новой записи.
        # Недостаточно просто иметь функцию, которая применяет вещи из вашего журнала между lastApplied и commitIndex,
        # остановившись, когда она достигает конца вашего журнала. Это связано с тем, что в вашем журнале могут быть записи,
        # отличающиеся от журнала лидера после тех записей, которые лидер отправил вам (которые все соответствуют вашему журналу).
        # Поскольку правило #3 гласит, что вы усекаете свой журнал только в случае конфликтующих записей,
        # они не будут удалены, и если leaderCommit находится за пределами записей, которые лидер отправил вам,
        # вы можете применить неверные записи.
        if leader_commit > self.commit_index: #фикисрованная длина не может получится больше чем у лидера (означает что у нас есть новые записи для фиксации, которые надо зафиксировать)
            self.commit_index = min(leader_commit, replicated_index)

        return AppendEntriesSucceeded(replicated_index) #а значит мы должны доставить новые зафиксированные сообщения
    ...
    def _update_committed_entries(self):
        ...
        majority_match_n = sorted(self.match_index)[self.num_servers // 2]  #majority_match_n соответствует наибольшему индексу записи в журнале, подтверждённой у большинства серверов.
        possible_ns = range(self.commit_index, min(majority_match_n, len(self.log)) + 1) #Это используется для проверки возможных записей, которые можно зафиксировать
        self._logger.info(f"possible entries that can be committed: {possible_ns}")
        n = None
        for possible_n in possible_ns: #Если существует запись, которую большинство серверов подтверждает и которая еще не была зафиксирована, то ее можно зафиксировать
    ...
    def _send(self, to, content):
        self.write()
        self.outbox.put( #помещаем в очередь
            Message(
