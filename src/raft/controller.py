...
class RaftController:
    def __init__(self, server_no, machine, net):
        ...
        self.hb_since_election_timeout_lock = threading.Lock() #блокировка для отслеживания времени с последней отправки heartbeat-сигнала после истечения таймаута выборов
    ...
    def _rcv_message_loop(self):
        while True:
            msg = self._net.recv() #из очереди inbox всё берём потихоньку
            self._events.put(("message", msg))

    def _handle_machine_events(self):
        while True:
            raft_server_event = self._machine.outbox.get() #из очереди outbox всё берём потихоньку
    ...
    def _event_loop(self):
        while True:
            self._pause_signal.wait() #ждет пока его кто-то не пнёт
